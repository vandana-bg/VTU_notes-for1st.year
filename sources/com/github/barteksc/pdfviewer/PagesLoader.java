...
class PagesLoader {
    ...
    private Pair<Integer, Integer> getPageColsRows() {
        ...
        return new Pair<>(Integer.valueOf(MathUtils.ceil(1.0f / ((Constants.PART_SIZE * (1.0f / this.pdfView.getOptimalPageWidth())) / this.pdfView.getZoom()))), Integer.valueOf(MathUtils.ceil(1.0f / partHeight)));
    }
    ...
    private int loadRelative(int number, int nbOfPartsLoadable, boolean belowView) {
        ...
        int loaded;
        int i = number;
        int i2 = nbOfPartsLoadable;
        if (this.pdfView.isSwipeVertical()) {
            newOffset = (this.pdfView.getCurrentYOffset() - ((float) (belowView ? this.pdfView.getHeight() : 0))) - ((this.rowHeight * ((float) i)) + 1.0f);
        } else {
            newOffset = (this.pdfView.getCurrentXOffset() - ((float) (belowView ? this.pdfView.getWidth() : 0))) - (this.colWidth * ((float) i));
        }
    ...
    public int loadVisible() {
        ...
        if (this.pdfView.isSwipeVertical()) {
            ...
            if (firstHolder.page == lastHolder.page) {
                ...
                visibleRows = lastHolder.row + 1 + visibleRows2;
            }
            ...
            if (firstHolder.page == lastHolder2.page) {
                ...
                visibleCols = lastHolder2.col + 1 + visibleCols2;
            }
