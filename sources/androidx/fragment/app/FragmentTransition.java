...
class FragmentTransition {
    ...
    public static void calculateFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isReordered) {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            addToFirstInLastOut(transaction, (FragmentTransaction.C0399Op) transaction.mOps.get(opNum), transitioningFragments, false, isReordered);
        }
    }
    ...
    public static void calculatePopFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isReordered) {
        if (transaction.mManager.getContainer().onHasView()) {
            for (int opNum = transaction.mOps.size() - 1; opNum >= 0; opNum--) {
                addToFirstInLastOut(transaction, (FragmentTransaction.C0399Op) transaction.mOps.get(opNum), transitioningFragments, true, isReordered);
            }
        }
    }
    ...
    private static void addToFirstInLastOut(BackStackRecord transaction, FragmentTransaction.C0399Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isReorderedTransaction) {
        int containerId;
        boolean setLastIn;
        boolean setFirstOut;
        BackStackRecord backStackRecord = transaction;
        FragmentTransaction.C0399Op op2 = op;
